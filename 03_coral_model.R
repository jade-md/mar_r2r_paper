...
dir.create(file.path("06_spatial_predictions", folder, subfolder), showWarning=TRUE, recursive=TRUE)
...
raster_list <- list.files(file.path("05_predictors", folder, subfolder), pattern="tif$")
raster_list
# rename the TSS value to the file name
final_vars[3] <- "tss_s0_clim0"
final_vars
...
raster_stack <- stack(file.path("05_predictors", folder, subfolder, raster_list))
...
# Export to geotiff
writeRaster(raster_prediction_backtransformed, filename=file.path("06_spatial_predictions", folder, subfolder, paste(scenario, response_var,sep="_")), format="GTiff", overwrite=TRUE)
writeRaster(raster_prediction_backtransformed, filename=file.path("07_validation", scenario, paste(model, scenario, response_var, scale, sep="_")), format="GTiff", overwrite=TRUE)
...
# predict coral cover under scenario
dir.create(file.path("06_spatial_predictions", folder, subfolder, "raw_predictions"), showWarning=TRUE, recursive=TRUE)
raster_list <- list.files(file.path("05_predictors", folder, subfolder), pattern="tif$")
raster_list
final_vars[3] <- "tss_s1_clim0"
final_vars
raster_list <- raster_list[match(paste(final_vars,".tif",sep=""), raster_list)]
raster_list
raster_stack <- stack(file.path("05_predictors", folder, subfolder, raster_list))
...
raster_prediction <- predict(raster_stack, gbm_model_final, n.trees=gbm_model_final$gbm.call$best.trees, progress="text")
raster_prediction_backtransformed <-  Back.Transform.Prediction(raster_prediction)
writeRaster(raster_prediction_backtransformed, filename=file.path("06_spatial_predictions", folder, subfolder, "raw_predictions", paste(scenario, response_var,sep="_")), format="GTiff", overwrite=TRUE)

###### correct coral cover to smooth the relationship ####
s0_clim0_cor <- raster("06_spatial_predictions/coral/2022/s0_clim0_cor.tif")
s1_clim0_cor <- raster("06_spatial_predictions/coral/2022/raw_predictions/s1_clim0_cor.tif")
s1_clim0_cor_dlt <- s1_clim0_cor-s0_clim0_cor
# conditional statement
s1_clim0_cor_e <- s1_clim0_cor
s1_clim0_cor_e[] = ifelse(s1_clim0_cor_dlt[]<0, s0_clim0_cor[], s1_clim0_cor[])
# check
s1_clim0_cor_dlt <- s1_clim0_cor_e-s0_clim0_cor
plot(s1_clim0_cor_dlt)
writeRaster(s1_clim0_cor_e, filename=file.path("06_spatial_predictions", folder, subfolder, paste(scenario, response_var,sep="_")), format="GTiff", overwrite=TRUE)
writeRaster(s1_clim0_cor_dlt, filename=file.path("09_indicators_delta/2022", paste("cor_s1_clim0_delta.tif")), format="GTiff", overwrite=TRUE)
...
###### correct coral cover to smooth the relationship ####
s0_clim0_cor <- raster("06_spatial_predictions/coral/2022/s0_clim0_cor.tif")
s2_clim0_cor <- raster("06_spatial_predictions/coral/2022/raw_predictions/s2_clim0_cor.tif")
s2_clim0_cor_dlt <- s2_clim0_cor-s0_clim0_cor
# conditional statement
s2_clim0_cor_e <- s2_clim0_cor
s2_clim0_cor_e[] = ifelse(s2_clim0_cor_dlt[]>0, s0_clim0_cor[], s2_clim0_cor[])
# check
s2_clim0_cor_dlt <- s2_clim0_cor_e-s0_clim0_cor
plot(s2_clim0_cor_dlt)
writeRaster(s2_clim0_cor_e, filename=file.path("06_spatial_predictions", folder, subfolder, paste(scenario, response_var,sep="_")), format="GTiff", overwrite=TRUE)
writeRaster(s2_clim0_cor_dlt, filename=file.path("09_indicators_delta/2022", paste("cor_s2_clim0_delta.tif")), format="GTiff", overwrite=TRUE)
...
###### correct coral cover to smooth the relationship ####
s0_clim0_cor <- raster("06_spatial_predictions/coral/2022/s0_clim0_cor.tif")
s3_clim0_cor <- raster("06_spatial_predictions/coral/2022/raw_predictions/s3_clim0_cor.tif")
s3_clim0_cor_dlt <- s3_clim0_cor-s0_clim0_cor
# conditional statement
s3_clim0_cor_e <- s3_clim0_cor
s3_clim0_cor_e[] = ifelse(s3_clim0_cor_dlt[]<0, s0_clim0_cor[], s3_clim0_cor[])
# check
s3_clim0_cor_dlt <- s3_clim0_cor_e-s0_clim0_cor
plot(s3_clim0_cor_dlt)
writeRaster(s3_clim0_cor_e, filename=file.path("06_spatial_predictions", folder, subfolder, paste(scenario, response_var,sep="_")), format="GTiff", overwrite=TRUE)
writeRaster(s3_clim0_cor_dlt, filename=file.path("09_indicators_delta/2022", paste("cor_s3_clim0_delta.tif")), format="GTiff", overwrite=TRUE)
...
raster_list <- list.files(file.path("05_predictors", folder, subfolder), pattern="tif$")
raster_list
final_vars[3] <- "tss_s4a_clim0_bgh_mes_max"
final_vars
raster_list <- raster_list[match(paste(final_vars,".tif",sep=""), raster_list)]
raster_list
raster_stack <- stack(file.path("05_predictors", folder, subfolder, raster_list))
...
###### correct coral cover to smooth the relationship ####
s0_clim0_cor <- raster("06_spatial_predictions/coral/2022/s0_clim0_cor.tif")
s4a_clim0_cor <- raster("06_spatial_predictions/coral/2022/raw_predictions/s4a_clim0_bgh_mes_max_cor.tif")
s4a_clim0_cor_dlt <- s4a_clim0_cor-s0_clim0_cor
# conditional statement
s4a_clim0_cor_e <- s4a_clim0_cor
s4a_clim0_cor_e[] = ifelse(s4a_clim0_cor_dlt[]<0, s0_clim0_cor[], s4a_clim0_cor[])
# check
s4a_clim0_cor_dlt <- s4a_clim0_cor_e-s0_clim0_cor
plot(s4a_clim0_cor_dlt)
writeRaster(s4a_clim0_cor_e, filename=file.path("06_spatial_predictions", folder, subfolder, paste(scenario, response_var,sep="_")), format="GTiff", overwrite=TRUE)
...
raster_list <- list.files(file.path("05_predictors", folder, subfolder), pattern="tif$")
raster_list
final_vars[3] <- "tss_s4b_clim0_mar_mes_max"
final_vars
raster_list <- raster_list[match(paste(final_vars,".tif",sep=""), raster_list)]
raster_list
raster_stack <- stack(file.path("05_predictors", folder, subfolder, raster_list))
...
###### correct coral cover to smooth the relationship #### 
s0_clim0_cor <- raster("06_spatial_predictions/coral/2022/s0_clim0_cor.tif")
s4b_clim0_cor <- raster("06_spatial_predictions/coral/2022/raw_predictions/s4b_clim0_mar_mes_max_cor.tif")
s4b_clim0_cor_dlt <- s4b_clim0_cor-s0_clim0_cor
# conditional statement
s4b_clim0_cor_e <- s4b_clim0_cor
s4b_clim0_cor_e[] = ifelse(s4b_clim0_cor_dlt[]<0, s0_clim0_cor[], s4b_clim0_cor[])
# check
s4b_clim0_cor_dlt <- s4b_clim0_cor_e-s0_clim0_cor
plot(s4b_clim0_cor_dlt)
writeRaster(s4b_clim0_cor_e, filename=file.path("06_spatial_predictions", folder, subfolder, paste(scenario, response_var,sep="_")), format="GTiff", overwrite=TRUE)
writeRaster(s4b_clim0_cor_dlt, filename=file.path("09_indicators_delta/2022", paste("cor_s4b_clim0_mar_mes_max_cor_delta.tif")), format="GTiff", overwrite=TRUE)
